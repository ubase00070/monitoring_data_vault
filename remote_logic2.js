(function() {
   ...
   ============================================================ */
	...
	// ── 게임패드 D-pad 마스킹 (게임패드 ON 시 뉴비고 맵 이동 차단) ──
	const _originalGetGamepads = navigator.getGamepads.bind(navigator);
	navigator.getGamepads = () => {
	    const gamepads = _originalGetGamepads();
	    return gamepads.map(gp => {
	        ...
	        if (!isGamepadOn) return gp;
	        const buttons = gp.buttons.map((btn, i) => {
	            if ([12, 13, 15].includes(i)) {
	                return { pressed: false, touched: false, value: 0 };
	            }
	            return btn;
	        });
	        return Object.assign(Object.create(Object.getPrototypeOf(gp)), gp, { buttons });
	    });
	};
	...
	setInterval(() => {
	    const gp = _originalGetGamepads()[0]; // 마스킹 전 원본으로 읽기
	    if (!gp) return;
