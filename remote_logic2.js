   ...
   ============================================================ */
    ...
    window.addEventListener('keydown', (e) => {
        ...
        window.openSecretOverlay = async function() {
            ...
            // 실시간 컨트롤러 테스터 — 사용자가 벡터화한 xbox_skeleton.svg 원본 그대로 + 버튼별 투명 오버레이(누르면 검정으로 채워짐) + 스틱 기울임 시 캡 이동
            window.openGamepadTesterOverlay = function() {
                const box = document.createElement('div');
                box.style.cssText = `background:#1e1e2e; color:#e2e8f0; border-radius:16px; padding:16px; width:100%; box-sizing:border-box; max-height:85vh; overflow-y:auto; box-shadow:0 10px 50px rgba(0,0,0,0.7); pointer-events:auto;`;
                box.innerHTML = `
                    ...
                    <div style="background:#f4ede0; border-radius:12px; padding:8px; max-width:360px; margin:0 auto;">
